...
public class GetMainMemberByIdQueryHandler : IRequestHandler<GetMainMemberByIdQuery, Result<MainMemberResponse>>
{
    ...
    public async Task<Result<MainMemberResponse>> Handle(GetMainMemberByIdQuery request,
        CancellationToken cancellationToken)
    {
        ...
        {
            return Result<MainMemberResponse>.Error(new Error("Get Main Member", "Main member not found"));
        }

        return Result<MainMemberResponse>.Success(mainMember);
    }
}