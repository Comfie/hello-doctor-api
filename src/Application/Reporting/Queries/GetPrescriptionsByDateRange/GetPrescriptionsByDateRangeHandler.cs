...
public class GetPrescriptionsByDateRangeHandler : IRequestHandler<GetPrescriptionsByDateRangeQuery, Result<PaginatedPrescriptionsResponse>>
{
    private readonly IApplicationDbContext _db;
    private readonly IDateTimeService _dateTime;
    private readonly IUser _user;

    public GetPrescriptionsByDateRangeHandler(IApplicationDbContext db, IDateTimeService dateTime, IUser user)
    {
        _db = db;
        _dateTime = dateTime;
        _user = user;
    ...
    public async Task<Result<PaginatedPrescriptionsResponse>> Handle(GetPrescriptionsByDateRangeQuery request, CancellationToken ct)
    {
        ...
        var query = _db.Prescriptions
            ...
            .Where(p => p.AssignedPharmacyId == request.PharmacyId
                && p.Created >= request.StartDate
                && p.Created <= request.EndDate);
